...
for l in range(max_len + 1):
    for p in Permutations(l):
        if not perm_prop(p):
            for xsep in choose(l - 1, n - 1):
                for ysep in choose(l - 1, m - 1):
                    arr = [ [ [] for j in range(m) ] for i in range(n) ]

                    for i in range(n):
                        for j in range(m):
                            for k in range(0 if j == 0 else ysep[j-1] + 1, (l - 1 if j == m - 1 else ysep[j]) + 1):
                                if (0 if i == 0 else xsep[i-1] + 1) <= l - p[k] <= (l - 1 if i == n - 1 else xsep[i]):
                                    arr[i][j].append(p[k])
...
#         r, c = divmod(i, m)
#         if (any_empty_set and cur[i] != 13) or (empty_rows and r not in empty_rows and empty_cols and c not in empty_cols):
#             continue
# 
...
# 
#         visited = set()
#         walk(n, m, tuple([ 13 for i in range(n) for j in range(m) ]))
# 
